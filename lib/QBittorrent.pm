...
sub get_torrents_infohash {
    ...
    Logger::info("[SUMMARY] Torrents loaded in qBittorrent     \t" . scalar(keys %hash));
...
sub get_q_zombies {
    ...
    if ($opts->{dev_mode} && !$opts->{scan_zombies}) {
        ...
        if (@cache_files) {
            ...
            Logger::info("[SUMMARY][CACHE] Zombie torrents in qBittorrent:\t" . scalar keys %$cached);
            return $cached;
        }
        else {
    ...
    Logger::info("[SUMMARY][LIVE] Zombie torrents in qBittorrent:\t" . scalar keys %zombies);
